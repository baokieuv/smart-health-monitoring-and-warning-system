  ...
  const load = async () => {
    ...
    try {
      ...
      if (res?.status === 'success') setData(res.data)
        ...
        // fallback mock data when API returns non-success
        setData(mockDoctors(page, limit, search, specialization))
      }
      // Best-effort: fetch position options (backend may return specializations)
      try {
    ...
    <div>
      <div className="toolbar">
        <div className="filters">
          {/* Search is now in the header (shared). Keep only position filter here. */}
          ...
          <tbody>
            {data.doctors?.map((d) => (
              <tr key={d.id}>
                ...
                <td>
                  <Link className="btn ghost" to={routers.AdminDoctorDetail(d.id)} style={{ marginRight: 8 }}>Sửa</Link>
                  <button className="btn danger" onClick={() => onDelete(d.id)}>Xóa</button>
                </td>
              </tr>
            ))}
            {data.doctors?.length === 0 && !loading && (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 16, color: 'var(--muted)' }}>Không có dữ liệu</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {loading && <div style={{ marginTop: 8, color: 'var(--muted)' }}>Đang tải…</div>}
      {error && (
